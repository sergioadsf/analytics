...
public class GAVisit extends GA {
	...
	private GAVisit(Ga ga, String token, String idTable, String dateInit1, String dateEnd1, String dateInit2,
			String dateEnd2) {
		super(ga, token, idTable, dateInit1, dateEnd1, dateInit2, dateEnd2);
	}

	public static GAVisit start(ParamsDTO params) throws IOException, Exception {
		Ga ga = Connection.getAnalytics(params.getToken()).data().ga();
		return new GAVisit(ga, params.getToken(), params.getIdProfile(), params.getDateInit1(), params.getDateEnd1(),
				params.getDateInit2(), params.getDateEnd2());
	}

	public GAVisit visit() {
		this.visit(super.maxResults);
		return this;
	}

	public GAVisit visit(Integer maxResult) {
		super.metrics = String.format("%s,%s,%s", GAParams.SESSOES, GAParams.TAXA_DE_REJEICAO,
				GAParams.TEMPO_MEDIO_SESSAO);
		super.setDimension(GAParams.DIM_VISITA_TIPO);
		super.setSort(GAParams.SESSOES);
		return this;
	}

	public ListVisitDTO execute() throws Exception {

		ListVisitDTO list = ListVisitDTO.start(execute(dateInit1, dateEnd1));
	...
	private List<VisitDTO> execute(String dateInit, String dateEnd) throws Exception {
		if (ga == null) {
			throw new Exception("");
		}
	...
	private List<VisitDTO> convertList(GaData results) {
		...
		if (results.getRows() != null) {
			for (List<String> listaStr : results.getRows()) {
				...
				list.add(new VisitDTO(values[NAME].toString(), values[QUANTITY_ACCESS].toString(),
